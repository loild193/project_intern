...
function UserDetail({ open }) {
	const { userState: { user }, getUserDetail } = useContext(UserContext);
	const { id } = useParams();
	...
	const handleGotoEditUserPage = () => {
		history.replace(`/admin/user/edit/${id}`)
	}
      ...
      <div className={classes.drawerHeader}>
				<div className="detail__wrapper">
					<div className="detail__top">
						<h4>User detail</h4>
						<Button variant="contained" color="secondary" onClick={handleGotoEditUserPage}>
							<EditIcon />
							Edit
						</Button>
					</div>
					...
					<div className="detail__bottom">
						{
							...
							<Grid container>
								...
								<Grid item xs={6}>
									<p>Role: { changeRoleToName(user?.role) }</p>
								</Grid>
								<Grid item xs={12}>
									<p>Part: { changeBophanToName(user?.bophan_id) }</p>
								</Grid>
							</Grid>
						}
					</div>
